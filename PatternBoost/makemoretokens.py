...
class CharDataset(Dataset):
    ...
    def decode(self, ix):
        # Add error handling for out-of-vocabulary tokens
        decoded_tokens = []
        for i in ix:
            if i < len(self.itos):
                decoded_tokens.append(self.itos[i])
            else:
                # Handle out-of-vocabulary tokens by using a fallback
                decoded_tokens.append(f"V{i}")  # Use the token ID as fallback
        word = ','.join(decoded_tokens)
        return word
